...
public enum Location {
    ...
    public static void check() {
        ...
        // Location-based
            ...
            if (Objects.requireNonNull(HideawayPlus.player().getServer()).getCustomBossEvents().getEvents().stream().findFirst().isPresent()) {
                if (scoreboardContains("\ue4d5")) {
                    HideawayPlus.setLocation(KING_OF_CASTLE);
                }
                else if (scoreboardContains("\ue4db")) {
    ...
    private static boolean playerIsInRadius(Vec3 loc, int radius) {
        Vec3 actualLoc = HideawayPlus.player().position();
    ...
    private static boolean bossbarContains(String content) {
        BossEvent bar = HideawayPlus.player().getServer().getCustomBossEvents().getEvents().stream().findFirst().get();
