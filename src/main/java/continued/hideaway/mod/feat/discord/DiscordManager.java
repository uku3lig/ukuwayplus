...
public class DiscordManager {
    ...
    public DiscordManager start() {
        if (!active) {
            HideawayContinued.logger().info("Starting Discord RPC client...");
            client = new IPCClient(1136888078510858323L);
            client.setListener(new IPCListener() {
                @Override
                public void onReady(IPCClient client) {
                    HideawayContinued.logger().info("Discord RPC client connected!");
                    active = true;
                    start = Instant.now();
                    update();
                }
    ...
    public void update() {
        if (active) {
            Location loc = HideawayContinued.location();
            ...
            builder.setState(loc.description)
                    ...
                    .setLargeImage(loc.largeIcon.key(), "HideawayContinued v" + HideawayContinued.version())
